...
class CustomerCheckoutController extends Controller
{
    public function index(Request $request)
    {
        try {
            ...
            return back()->with('error', 'Terjadi kesalahan saat memuat halaman checkout: ' . $e->getMessage());
        }
    }
    ...
    public function store(Request $request)
    {
        try {
            ...
            foreach ($cartOrders as $order) {
                ...
                // Update order status
                $order->status_payment = Orders::STATUS_PAYMENT_PENDING;
                $order->status_delivery = Orders::STATUS_DELIVERY_PENDING;
                $order->save();
