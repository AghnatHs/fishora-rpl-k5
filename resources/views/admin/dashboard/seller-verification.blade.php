        ...
        <div class="px-2">
            <ul role="list" class="space-y-4">
                @forelse ($sellers as $seller)
                    <li>
                        <div class="bg-gradient-to-br from-white to-[#EBF1FA] rounded-lg p-4 shadow-sm border-2 border-[#4871AD]/10">
                            <div class="flex flex-col space-y-3">
                                <div class="flex justify-between items-start">
                                    <p class="text-lg font-serif font-medium text-[#4871AD]">{{ $seller->shop_name }}</p>
                                ...
                                <div class="grid grid-cols-2 gap-2">
                                    <div class="flex flex-col">
                                        ...
                                        <img src="data:{{ $seller->ktp_mime }};base64,{{ base64_encode($seller->ktp) }}"
                                            alt="ID Card" class="w-full h-auto border border-[#4871AD]/20 rounded-md shadow-sm">
                                    </div>
                                    <div class="flex flex-col">
                                        ...
                                        <img src="data:{{ $seller->proof_of_business_mime }};base64,{{ base64_encode($seller->proof_of_business) }}"
                                            alt="Business Proof" class="w-full h-auto border border-[#4871AD]/20 rounded-md shadow-sm">
                                    </div>
                                </div>
                                
                                @if (!$seller->admin_verified_at)
                                    <div class="flex space-x-3 pt-2">
                                        <form action="{{ route('admin.dashboard.seller-verification.post', ['seller' => $seller]) }}" method="POST">
                                            @csrf
